...
void GameObjects::CollisionMap::add_collider(int x, int y, int src_x, int src_y)
{
	...
	const auto collision = &collider.add_component<Components::Collision>("aabb", new Colliders::Polygon(&transform->position, glm::vec2(half_scaled_size, half_scaled_size), { glm::vec2(-half_scaled_size, -half_scaled_size), glm::vec2(half_scaled_size, -half_scaled_size), glm::vec2(half_scaled_size, half_scaled_size), glm::vec2(-half_scaled_size, half_scaled_size) }));
